    ...
    <main className="max-w-6xl mx-auto px-4 py-16 text-slate-900 space-y-12">
      <header className="space-y-3">
        ...
        <p className="text-slate-600 text-sm md:text-base">
          すべて note の一次データです。症状タグごとにどの行動が語られているか、改善・経過・悪化のバランスとともに確認できます。
        </p>
      </header>
  ...
  return (
    <section className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-6 py-5 border-b border-slate-100">
        ...
        <p className="text-sm text-slate-500 mt-1">成功率 = 改善件数 / 総件数 ・最新 30 件から算出</p>
      </div>
        ...
        <table className="min-w-full text-sm text-slate-700">
          <thead>
            <tr className="text-left border-b border-slate-100">
              ...
              <th className="py-3 px-6">主に語られている症状タグ</th>
              ...
              return (
                <tr key={method.method_slug} className="border-b border-slate-100">
                  <td className="py-4 px-6">
                    ...
                    {method.sample && (
                      <p className="text-slate-500 line-clamp-2 text-xs mt-1">{method.sample.raw_posts.content}</p>
                    )}
                  </td>
                  <td className="py-4 px-6 text-emerald-600 font-semibold">{method.successRate}%</td>
                  <td className="py-4 px-6 text-slate-600">
                    {method.positive} / {method.neutral} / {method.negative}
                  </td>
                  <td className="py-4 px-6 text-slate-600">
                    {tags.length === 0 ? '—' : tags.slice(0, 3).map((tag) => `#${tag}`).join(' 、 ')}
                  </td>
                  <td className="py-4 px-6 text-slate-600">{formatDate(method.lastReportedAt)}</td>
                  <td className="py-4 px-6">
                    <Link href={`/method/${method.method_slug}`} className="text-emerald-600 hover:text-emerald-700">
                      ページへ →
                    </Link>
                  </td>
                </tr>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {symptoms.slice(0, 6).map((symptom) => (
          <article key={symptom.keyword} className="rounded-2xl bg-slate-50 p-4 border border-slate-200">
            <p className="text-sm text-slate-500">#{symptom.keyword}</p>
...
function buildMethodKeywordMap(events: NoteEvent[]): Record<string, string[]> {
  ...
  for (const event of events) {
    const keyword = event.raw_posts.source_keyword?.trim();
    if (!keyword) continue;
